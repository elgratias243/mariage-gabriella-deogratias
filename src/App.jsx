...
import { getFirestore, collection, addDoc, doc, getDoc, setDoc, increment } from "firebase/firestore";
...
const HERO_PHOTO = "https://ouwxiqtaoprnwl63.public.blob.vercel-storage.com/IMG_6365.JPEG";
const BG_PHOTOS = [
...
async function saveRSVP(guestData, accepted) {
  try {
    await addDoc(collection(db, "rsvp-responses"), {
...
async function incrementVisits() {
  try {
    ...
    console.error("Visit increment error:", e);
  }
}
...
function CouplePhoto({ onContinue }) {
  ...
  return (
    <section style={{
      ...
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(10,10,10,0.3) 0%, rgba(10,10,10,0.6) 100%)" }} />
      ...
      <div style={{
        ...
        <button onClick={onContinue} style={{
          ...
          cursor: "pointer", transition: "all .4s ease", boxShadow: "0 4px 20px rgba(0,0,0,0.3)",
        }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ width: 3, height: 8, borderRadius: 2, background: C.goldLight, animation: "scrollBounce 1.8s ease infinite" }} />
        </div>
      </div>
